...
const finalizarPedido = () => {
    seleciona('.cart--finalizar').addEventListener('click', () => {
        ...
        // Construir a mensagem com os detalhes do pedido
        let mensagemPedido = "Olá! Abaixo está o seu pedido:\n";
        let subtotal = 0; // Inicializar subtotal

        cart.forEach((item, index) => {
            let pizzaItem = pizzaJson.find(pizza => pizza.id == item.id);
            let tamanhoPizza = pizzaItem.sizes[item.size]; // Acessar o tamanho correto da pizza
            let precoUnitario = item.price; // Preço unitário do item
            let precoTotalItem = precoUnitario * item.qt; // Preço total do item
            subtotal += precoTotalItem; // Adicionar ao subtotal
            mensagemPedido += `\nItem ${index + 1}:\n`;
            mensagemPedido += `Pizza: ${pizzaItem.name}\n`;
            mensagemPedido += `Tamanho: ${tamanhoPizza} (${item.size.toUpperCase()})\n`; // Adicionando o tamanho (P, M, G)
            mensagemPedido += `Quantidade: ${item.qt}\n`;
            mensagemPedido += `Preço Unitário: ${formatoReal(precoUnitario)}\n`;
            mensagemPedido += `Subtotal: ${formatoReal(precoTotalItem)}\n`;
        });

        mensagemPedido += `\nSubtotal: ${formatoReal(subtotal)}\n`; // Adicionar subtotal à mensagem

        // Substituir espaços em branco por %20 para a URL do WhatsApp
        mensagemPedido = encodeURIComponent(mensagemPedido);

        // Redirecionar para o WhatsApp com a mensagem do pedido
        window.location.href = `https://wa.me/5531995461381?text=${mensagemPedido}`;
    });
}
